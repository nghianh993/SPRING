...
<script src="<c:url value="/static/custom/js/action/action.js"/>"></script>
							...
							<li class="dd-item">
								<div class="dd-handle"> ${ item.name}
									<div class="pull-right action-buttons">
										<a class="blue btnaddaction" href="javascript: ;" data-id="${item.id }" title="Thêm chức năng"> 
								...
								<c:if test="${ item.getActions() != null && item.getActions().size() > 0 }">
									<ol class="dd-list">
										<c:forEach var="action" items="${item.getActions() }">
											<li class="dd-item">
													<div class="dd-handle operation">
														${ action.link }
														<div class="pull-right action-buttons">
															<a class="blue btneditac" href="javascript: ;" title="Sửa" data-id="${action.id }"> 
																<i class="ace-icon fa fa-pencil bigger-130"></i>
															</a> 
															<c:if test="${!action.islock}">
																<a data-id="${action.id }" class="red btnlockac" href="javascript: ;" title="Khóa chức năng" > 
																	<i class="ace-icon fa fa-unlock-alt bigger-130"></i>
																</a>
															</c:if>
															
															<c:if test="${action.islock}">
																<a data-id="${action.id }" class="red btnunlockac" href="javascript: ;" title="Mở khóa chức năng" > 
																	<i class="ace-icon fa fa-lock bigger-130"></i>
																</a>
															</c:if>
															
															<a class="red btndelac" href="javascript: ;" title="Xóa chức năng" data-id="${action.id }"> 
																<i class="ace-icon fa fa-trash-o bigger-130"></i>
															</a>
														</div>
													</div>
...
<!-- Modal -->
<div class="modal fade" id="actionModal" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="frmFormAction">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <select class="form-control slgroup slgrac" name="slgrac">
                            	<c:forEach var="entry" items="${lstData}"> 
                            		<c:if test="${entry.value != null && entry.value.size() > 0}">
                            			<c:forEach var="item" items="${ entry.value }">
                        ...
                        <button class="btn btn-sm btn-success btnaddgroup" title="Thêm mới nhóm chức năng">

		...
		submitHandler: function () {
			...
			if ($(".btnGroupModal").hasClass("btn_add")) {
				...
				if ($(".btnGroupModal").hasClass("btn_update")) {
					...
					$.LoadingOverlay("hide");
					if (resp.result) {
						fis_vnp_js.fis_vnp().showmessage("Cập nhật thông tin nhóm quyền thành công", "success");
						$("#goupModal").modal("hide");
					} else {
						fis_vnp_js.fis_vnp().showmessage("Hệ thống gặp sự cố khi thêm mới dữ liệu. Vui lòng thao tác lại!!!", "error");
					}
		...
		$.post(root + "/api/admin/permission/groupdetail", data, function (resp) {
			...
			if (resp.result) {
				if (resp.result.resultData !== null) {
					...
					fis_vnp_js.fis_vnp().showmessage("Không tìm thấy dữ liệu bạn yêu cầu. Vui lòng thao tác lại!!!", "error");
				}
			} else {
				fis_vnp_js.fis_vnp().showmessage("Hệ thống gặp sự cố khi thêm lấy dữ liệu. Vui lòng thao tác lại!!!", "error");
			}
